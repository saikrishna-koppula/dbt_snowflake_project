...
suppliers = pd.DataFrame({
    ...
    "supplier_name": [fake.company() for _ in range(supplier_count)],
...
products = pd.DataFrame({
    ...
    "product_name": [fake.bs().title() for _ in range(product_count)],
    "category": [random.choice(['Electronics', 'Furniture', 'Office Supplies', 'Food']) for _ in range(product_count)],
    "unit_price": np.round(np.random.uniform(5.0, 500.0, product_count), 2)
})
...
purchase_orders['quantity'] = np.random.randint(1, 100, po_count)
...
payments_df['paid_amount'] = np.round(payments_df['amount_due'] * np.random.uniform(0.8, 1.0, payment_count), 2)
